 ...
 *
 * 虚拟机栈：Stack Frame 栈帧
 * 程序计数器（Program Counter）：描述执行的顺序，执行到哪里了
 * 本地方法栈：主要用于执行本地方法
 ...
 * 哪些空间是可以使用的，哪些空间是已被使用的，接下来找出可以容纳下新创建对象的且未被使用的空间，在次空间存放该对象。
 * 同时还要修改列表上的记录）
 *
 * 对象在内存中的布局：
 * 1.对象头
...
public class MyTest1 {
    ...
    public static void main(String[] args) {
        // -Xms5m -Xmx5m -XX:+HeapDumpOnOutOfMemoryError OutOfMemoryError
